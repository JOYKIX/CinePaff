...
import { randomMovie, getMovieDiscoveryFilters } from './random-catalog.js?v=20260911-filters';
...
const isAniPaff = document.documentElement.dataset.app === 'anipaff';

const databasePaths = {
  media: isAniPaff ? 'anime' : 'movies',

  draw: isAniPaff ? 'animeDraw' : 'draw',

  history: isAniPaff
    ? 'animeDraw/history'
    : 'draw/history',

  screenings: isAniPaff
    ? 'animeScreenings'
    : 'screenings',

  screeningEvents: isAniPaff
    ? 'animeScreeningEvents'
    : 'screeningEvents',
...
function renderMovies() {
  ...
  document.querySelector('#clubStats').textContent = `${totalMovies} film${totalMovies > 1 ? 's' : ''} · ${allPrimary.length} membre${allPrimary.length > 1 ? 's' : ''}`;
  ...
  elements.movieList.replaceChildren(...(primaryList.length
    ...
    : [createEmptyState(selectionFilter === 'all' ? 'Aucun film en sélection' : 'Aucun film pour ce filtre')]));
...
function createEmptyState(text) {
  ...
  if (text === 'Aucun film en sélection' && canProposeMovie()) {
    const action = document.createElement('button');
    action.type = 'button';
    action.className = 'text-action';
    action.textContent = 'Ajouter un film';
...
function renderUsers() {
  ...
  elements.userList.replaceChildren(...list.map(account => {
    ...
    const mobileCount = document.createElement('small'); mobileCount.className = 'admin-user-mobile-count'; mobileCount.textContent = `${count} film${count > 1 ? 's' : ''}`; meta.append(mobileCount);
    ...
    activity.setAttribute('aria-label', `${count} film${count > 1 ? 's' : ''} proposé${count > 1 ? 's' : ''}`);
...
function renderSeenMovies() {
  ...
  elements.seenCount.textContent = `${allMovies.length} film${allMovies.length > 1 ? 's' : ''}`;
  elements.seenList.replaceChildren(...(list.length ? list.map(createSeenMovieCard) : [createEmptyState(allMovies.length ? 'Aucun résultat pour le moment' : 'Aucun film vu')]));
  if (activeSeenMovie) {
...
function createScreeningCard(event) {
  ...
  const details=document.createElement('button');details.type='button';details.className='text-action';details.textContent='Fiche du film';details.append(createIcon('arrow_forward'));details.addEventListener('click',()=>openRatingModal(event.movie,{allowRating:false}));
  content.append(time,title,host,details);
...
async function searchScreeningMovies() {
  ...
  try{
    const response=await fetch(`https://api.themoviedb.org/3/search/movie?query=${encodeURIComponent(query)}&language=fr-FR`,{headers:{Authorization:`Bearer ${tmdbToken}`,accept:'application/json'},signal:controller.signal});
    if(!response.ok)throw new Error('Search failed');
    const data=await response.json();if(controller.signal.aborted||query!==screeningUI.screeningQuery.value.trim())return;
    const list=(data.results||[]).slice(0,6);
    screeningUI.screeningSearchStatus.textContent=list.length?'':'Aucun film trouvé.';
...
// A deletion keeps its film/date snapshot in the queue for the Discord bot.
function screeningNotificationSnapshot(screening) {
  return {
    movie: {
      title: screening.movie.title,
      tmdbId: screening.movie.tmdbId || null,
...
async function saveScreening(event) {
  ...
  if(!screeningMovie){error.textContent='Choisis un film.';screeningUI.screeningQuery.focus();return;}
  ...
  try{
    ...
    if (!previous || previous.startsAt !== startsAt || previous.movie.tmdbId !== data.movie.tmdbId || previous.movie.title !== data.movie.title) {
  ...
  finally{screeningSaving=false;screeningUI.screeningForm.querySelectorAll('input,button').forEach(control=>control.disabled=false);screeningUI.saveScreening.removeAttribute('aria-busy');}
}
...
async function searchMovies(event = null) {
  ...
  try {
    ...
    status.textContent = results.length ? 'Choisis ton film' : 'Aucun film trouvé. Essaie un autre titre.';
  } catch (error) {
    if (controller.signal.aborted || query !== elements.movieQuery.value.trim()) return;
    elements.results.replaceChildren();
...
function createMovieButton(movie) {
  const alreadyAdded = proposedMovies().some(item => item.tmdbId === movie.id);
  const button = document.createElement('button');
  ...
  meta.textContent = alreadyAdded ? 'Déjà ajouté' : formatYear(movie.release_date) || 'Année inconnue';
...
async function proposeMovie() {
  ...
  try {
    ...
    setMessage('Impossible d’ajouter le film. Réessaie.');
  } finally {
    ...
    elements.confirmMovieSelection.querySelector('span').textContent = 'Ajouter à mes films';
...
function buildDrawCovers(list, selected = list[0]) {
  ...
  const nodes = Array.from({ length: count }, (_, index) => {
    ...
    image.addEventListener('error', () => { image.src = './image/logocinepafficon.svg'; }, { once: true });
    return image;
  });
...
async function drawMovie() {
  ...
  if (forcedMovieKey && !list.some(movie => movie.key === forcedMovieKey)) {
    renderForcedDrawOptions();
    setMessage('Ce film n’est plus disponible pour le tirage.');
    return;
  }
  drawInProgress = true;
...
function focusMovieSearch() {
  ...
  if (!canProposeMovie()) { setMessage('Tu ne peux pas proposer de film pour le moment.'); return; }
...
async function requestRandomMovie(path,signal) {
  const response=await fetch('https://api.themoviedb.org/3/'+path,{headers:{Authorization:'Bearer '+tmdbToken,accept:'application/json'},signal});
  if(!response.ok)throw new Error(response.status===429?'Catalogue occupé. Réessaie dans un instant.':'Catalogue indisponible. Réessaie.');
  return response.json();
}

function setupRandomDiscovery() {
  const ownDuplicate = movie => proposedMovies().some(existing => Number(existing.tmdbId)===Number(movie.id));
  const common = {
    kind: 'film',
    loadFilters: () => getMovieDiscoveryFilters(requestRandomMovie),
    createArtwork: movie => createPosterMedia(movieFromSearchResult(movie),'w342'),
    describe: movie => formatYear(movie.release_date),
  };
  const discover = (filters,{signal,recent},selection) => {
    ...
    return randomMovie(filters,{signal,exclude,request:requestRandomMovie});
  };
