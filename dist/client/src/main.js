...
const drawAnimationDuration = 6600;
...
const routeConfig = {
  ...
  draws: { label: 'Tirage', adminOnly: true },
...
function render() {
  ...
  elements.sessionAvatar.textContent = initials;
...
function renderMovies() {
  ...
  const countLabel = `${list.length} film${list.length > 1 ? 's' : ''}`;
  elements.selectionCount.textContent = countLabel;
  elements.drawPoolCount.textContent = countLabel;
  elements.drawButton.disabled = list.length === 0 || drawInProgress;
...
function renderDraw() {
  const isDrawing = elements.drawStage.classList.contains('is-drawing');
...
function buildDrawCovers(list, selected) {
  ...
  const count = Math.min(Math.max(pool.length * 12, 42), 68);
  const winnerIndex = Math.max(30, count - 10);
  const nodes = Array.from({ length: count }, (_, index) => {
    const movie = index === winnerIndex ? selected : pool[Math.floor(Math.random() * pool.length)];
    const image = document.createElement('img');
...
function playDrawAnimation(list, selected) {
  ...
  const duration = reducedMotion ? 500 : drawAnimationDuration;
  ...
  elements.drawButtonLabel.textContent = 'Tirage…';
  return new Promise((resolve) => {
    window.setTimeout(() => {
      elements.drawStatus.textContent = 'DERNIER TOUR';
    }, duration * 0.68);
    window.setTimeout(() => {
      elements.drawStatus.textContent = 'LE FILM';
      elements.drawStage.classList.remove('is-drawing');
...
async function drawMovie() {
  ...
  try {
    ...
    const remainingMovies = completed && !keepSelectionOnDraw ? list.length - 1 : list.length;
    elements.drawButton.disabled = remainingMovies <= 0;
  }
}
