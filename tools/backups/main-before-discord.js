...
async function deleteCurrentAccount() {
  ...
  try {
    ...
    Object.entries(data.screenings || {}).forEach(([key, event]) => {
      if (event.createdBy === deletedUser.id) changes[`screenings/${key}/createdBy`] = 'COMPTE SUPPRIMÉ';
    });
...
async function updateProfileId(event) {
  ...
  try {
    ...
    Object.entries(data.screenings || {}).forEach(([key, event]) => {
      if (event.createdBy === previousId) changes[`screenings/${key}/createdBy`] = nextId;
    });
    Object.entries(data.draw?.history || {}).forEach(([key, movie]) => {
...
async function saveScreening(event) {
  ...
  try{
    const data={movie:screeningMovie,startsAt,updatedAt:Date.now()};
    const write = editingScreeningKey
      ? update(ref(db,`screenings/${editingScreeningKey}`),data)
      : push(ref(db,'screenings'),{...data,createdAt:Date.now(),createdBy:currentUser.id});
    await Promise.all([write,new Promise(resolve=>window.setTimeout(resolve,500))]);
...
async function deleteScreening(key) {
  if(!canManageScreening(screenings[key]))return;
  try{await remove(ref(db,`screenings/${key}`));closeDeleteConfirmModal();setMessage('Soirée annulée');}
